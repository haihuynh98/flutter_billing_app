  ...
  Future<void> _onPrintReceipt(
    ...
    if (!printerHelper.isConnected) {
      ...
      if (savedMac != null) {
        ...
        if (!connected) {
          emit(state.copyWith(
              error: 'Tự động kết nối máy in thất bại!', clearError: false));
          emit(state.copyWith(clearError: true));
          return;
        }
      } else {
        emit(state.copyWith(
            error: 'Chưa kết nối máy in và không có máy in đã lưu!',
            clearError: false));
        emit(state.copyWith(clearError: true));
        return;
      }
    ...
    try {
      ...
      emit(state.copyWith(
          isPrinting: false, error: 'In hoá đơn thất bại: $e', clearError: false));
